...
defmodule Bitzy.CLI do
  require Logger

  def main(args) do
    Application.get_env(:bitzy, :master_node)
    |> Node.start

    Application.get_env(:bitzy, :slave_nodes)
  ...
  defp do_requests(n_requests, url, nodes) do
      ...
      |> Enum.flat_map(fn node ->
        1..req_per_node |> Enum.map(fn _ ->
          Task.Supervisor.async({Bitzy.TasksSupervisor, node}, BitzyWorker, :start, [url])
          end)
        end)
  ...
  defp do_help do
    IO.puts """
    Usage:
    bitzy -n [requests] [url]

    Options:
    -n, [--requests]

    Example:
    ./bitzy -n 100 http;//www.biberfever.com
    """
    System.halt(0)
  end
